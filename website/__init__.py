...
def create_app() -> Flask:
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = (
        "postgresql+psycopg2://{user}:{password}@{host}:{port}/{dbname}?sslmode=require".format(
...
class SubscriberForm(FlaskForm):
    ...
    email_id = EmailField(
        ...
        validators=[
            InputRequired(),
            Email(check_deliverability=True, message="E-mail address is not valid"),
        ],
    )
    password = PasswordField(
        "A strong password:", validators=[InputRequired(), Length(min=6, max=70)]
    )
...
class LoginForm(FlaskForm):
    username_or_email_id = StringField(
        "Your username or E-mail address:", validators=[InputRequired()]
    )
    password = PasswordField(
        "Your password:", validators=[InputRequired(), Length(min=6, max=70)]
    )
...
class PostForm(FlaskForm):
    category = SelectField(
        ...
        choices=[("Article"), ("Project"), ("Blog")],
    )
    author = SelectField(
        "Who is the author of this post?",
        validators=[InputRequired()],
        choices=[("Shashank Shetty"), ("Vibha P"), ("Srikanth Shetty"), ("Amulya B R")],
    )
    html_file = FileField(
        "The HTML file of the post:", validators=[FileRequired(), FileAllowed(["html"])]
    )
    url = StringField(
    ...
    images = MultipleFileField(
        "Accompanying images of the post:",
        validators=[
            FileRequired(),
            FileAllowed(["jpg", "jpeg", "png", "svg", "webp", "gif"]),
        ],
    )
    audios = MultipleFileField(
        "Audio files in the post (optional):",
        validators=[FileAllowed(["mp3", "ogg", "wav"])],
    )
...
class CommentForm(FlaskForm):
    rating = RadioField(
        "How much do you rate this post out of  7 ?",
